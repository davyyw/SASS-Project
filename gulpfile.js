...
// Fontawesome Task:
// 1. Copies the FA css files to the dist fonts folder
function fa(done){
    src(files.styles)
        .pipe(dest(folders.stylesDist)); 
    done();
...
// Build Task:     
exports.build = series(setBuildState, cleanTask, parallel(sassBuildTask, jsBuildTask, jQueryTask), parallel(imagesTask, mediaTask, fontsTask, fa), htmlTask, cacheBustTask);
